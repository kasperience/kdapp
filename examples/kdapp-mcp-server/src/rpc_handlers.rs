...
pub async fn handle_tools_call(request: Request, state: Arc<ServerState>) -> Response {
    let params = match request.params {
        Some(p) => p,
        None => {
            return Response::error(request.id, -32602, "Invalid params".to_string(), Some(serde_json::json!("Missing params")));
        }
    };
    let tool_name = match params.get("name").and_then(|n| n.as_str()) {
        Some(n) => n,
        None => {
            return Response::error(request.id, -32602, "Invalid params".to_string(), Some(serde_json::json!("Missing tool name")));
        }
    };
    ...
    let result = match tool_name {
        "kdapp_start_episode" => {
            let participants: Vec<String> = arguments
                .get("participants")
                ...
                .collect();
            match crate::tools::start_episode(state.clone(), participants).await {
        ...
        "kdapp_execute_command" => {
            ...
            match crate::tools::execute_command(state.clone(), episode_id, command, signature, signer).await {
                Ok(v) => v,
                Err(e) => {
                    return Response::error(
                        request.id,
        ...
        "kdapp_get_episode_state" => {
            let episode_id = arguments.get("episode_id").and_then(|v| v.as_str()).unwrap_or("").to_string();
            match crate::tools::get_episode_state(state.clone(), episode_id).await {
                Ok(v) => v,
                Err(e) => {
                    return Response::error(
                        request.id,
        ...
        "kdapp_get_agent_pubkeys" => match crate::tools::get_agent_pubkeys(state.clone()).await {
            Ok(v) => v,
            Err(e) => {
                return Response::error(
                    request.id,
        ...
        _ => {
            return Response::error(request.id, -32601, "Tool not found".to_string(), None);
        }
    };
