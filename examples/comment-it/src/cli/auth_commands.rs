...
use crate::utils::crypto::{load_private_key_from_file, parse_private_key};
...
pub async fn handle_authenticate(sub_matches: &ArgMatches) -> Result<(), Box<dyn Error>> {
    ...
    let timeout_seconds: u64 = sub_matches.get_one::<String>("timeout").unwrap().parse().map_err(|_| "Invalid timeout value")?;
