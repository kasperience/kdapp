...
#[derive(Deserialize)]
pub struct StatusQuery { pub pubkey: Option<String> }
...
async fn try_indexer_membership(episode_id: u64, pubkey: &str) -> Option<serde_json::Value> {
    ...
    let client = match reqwest::Client::builder().build() { Ok(c) => c, Err(_) => return None };
    let resp = match client.get(url).send().await { Ok(r) => r, Err(_) => return None };
    if !resp.status().is_success() { return None; }
    let json: serde_json::Value = match resp.json().await { Ok(j) => j, Err(_) => return None };
