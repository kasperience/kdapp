...
pub async fn verify_auth(State(state): State<PeerState>, Json(req): Json<VerifyRequest>) -> Result<Json<VerifyResponse>, StatusCode> {
    ...
    let _participant_addr = Address::new(Prefix::Testnet, Version::PubKey, &participant_pubkey.0.x_only_public_key().0.serialize());
    ...
    let auth_command = AuthCommand::SubmitResponse { signature: req.signature.clone(), nonce: req.nonce.clone() };
    ...
    let submission_result = match state
        .auth_http_peer
        .as_ref()
        .unwrap()
        .submit_episode_message_transaction(step, participant_wallet.keypair, participant_addr.clone(), utxo)
        .await
    {
        Ok(tx_id) => {
