...
pub async fn start_auth(State(state): State<PeerState>, Json(req): Json<AuthRequest>) -> Result<Json<AuthResponse>, StatusCode> {
    ...
    let new_episode = EpisodeMessage::<SimpleAuth>::NewEpisode { episode_id, participants: vec![participant_pubkey] };
    ...
    let submission_result = match state
        ...
        .as_ref()
        .unwrap()
        .submit_episode_message_transaction(new_episode, participant_wallet.keypair, participant_funding_addr.clone(), utxo)
        .await
    {
        Ok(tx_id) => {
