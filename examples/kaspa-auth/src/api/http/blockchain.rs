...
impl<'a> TxSubmitter<'a> {
    pub async fn submit_auth(
        ...
        let msg =
            EpisodeMessage::<SimpleAuth>::new_signed_command(episode_id as u32, cmd, self.server_keypair.secret_key(), client_pubkey);
        ...
        let addr = Address::new(Prefix::Testnet, Version::PubKey, &self.server_keypair.public_key().serialize()[1..]);
