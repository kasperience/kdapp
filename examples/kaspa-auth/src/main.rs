...
use kaspa_auth::{run_auth_server, AuthServerConfig};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match matches.subcommand() {
        Some(("test-episode", sub_matches)) => {
            let participant_count: usize = sub_matches.get_one::<String>("participants").unwrap().parse().unwrap_or(1);
        ...
        Some(("authenticate", sub_matches)) => {
            ...
            let timeout_seconds: u64 =
                sub_matches.get_one::<String>("timeout").unwrap().parse().map_err(|_| "Invalid timeout value")?;
        ...
        Some(("authenticate-full-flow", sub_matches)) => {
            ...
            let session_duration: u64 =
                sub_matches.get_one::<String>("session-duration").unwrap().parse().map_err(|_| "Invalid session duration value")?;
            let auth_timeout: u64 =
                sub_matches.get_one::<String>("auth-timeout").unwrap().parse().map_err(|_| "Invalid auth timeout value")?;
        ...
        Some(("logout", sub_matches)) => {
            let episode_id: u64 = sub_matches.get_one::<String>("episode-id").unwrap().parse().map_err(|_| "Invalid episode ID")?;

            let session_token = sub_matches.get_one::<String>("session-token").unwrap().clone();
        ...
        Some(("test-api", sub_matches)) => {
            let peer_url = sub_matches.get_one::<String>("peer").unwrap().clone();
            let command = kaspa_auth::cli::commands::test_api::TestApiCommand { peer: peer_url, verbose: false, json: false };
            command.execute().await?;
        }
        _ => {
...
fn test_episode_logic(participant_count: usize) -> Result<(), Box<dyn Error>> {
    ...
    let rollback1 = auth_episode.execute(&AuthCommand::RequestChallenge, Some(*pub_key), &metadata)?;
    ...
    let rollback2 = auth_episode.execute(
        &AuthCommand::SubmitResponse { signature: hex::encode(signature.0.serialize_der()), nonce: challenge },
        Some(*pub_key),
        &metadata,
    )?;
...
fn run_interactive_demo() -> Result<(), Box<dyn Error>> {
    ...
    let _rollback = auth_episode.execute(&AuthCommand::RequestChallenge, Some(alice_pk), &metadata)?;
...
async fn run_client_authentication(kaspa_signer: Keypair, auth_signer: Keypair) -> Result<(), Box<dyn Error>> {
    ...
    let new_episode = EpisodeMessage::<SimpleAuth>::NewEpisode { episode_id, participants: vec![client_pubkey] };
    ...
    let step = EpisodeMessage::<SimpleAuth>::new_signed_command(episode_id, auth_command, auth_signer.secret_key(), client_pubkey);
...
pub async fn run_http_coordinated_authentication(
    ...
    'blockchain_loop: loop {
        ...
        if let Ok(Some((received_episode_id, episode_state))) = recv_result {
            ...
            if received_episode_id == episode_id {
                if let Some(server_challenge) = &episode_state.challenge {
                    ...
                    println!(
                        "📡 Episode state update received, but no challenge yet. Auth status: {}",
                        episode_state.is_authenticated
                    );
                }
            } else {
        ...
        if attempt_count >= max_attempts {
            return Err(
                "❌ AUTHENTICATION FAILED: Could not retrieve challenge from blockchain within timeout. No HTTP fallback.".into()
            );
        }
    ...
    'auth_wait: loop {
        ...
        if wait_attempts >= max_wait_attempts {
            return Err(
                "❌ AUTHENTICATION FAILED: Could not retrieve session token from blockchain. Authentication incomplete.".into()
            );
        }
    ...
    Ok(AuthenticationResult { episode_id: episode_id.into(), session_token, authenticated: true })
}
...
pub async fn run_session_revocation(
    ...
    let auth_command = AuthCommand::RevokeSession { session_token: session_token.clone(), signature: signature_hex };

    // Step 5: Build transaction and submit to blockchain
    let episode_id_u32 = episode_id as u32; // Convert for kdapp framework
    let step = EpisodeMessage::<SimpleAuth>::new_signed_command(episode_id_u32, auth_command, auth_signer.secret_key(), client_pubkey);
    ...
    if wallet_file.exists() {
        ...
        match get_wallet_for_command(role, None) {
            Ok(wallet) => {
                ...
                let file_size = std::fs::metadata(&wallet_file).map(|m| m.len()).unwrap_or(0);
...
async fn run_authentication_with_timeout(
    ...
    if let Some(url) = peer_url {
        ...
        let auth_result =
            tokio::time::timeout(timeout_duration, run_http_coordinated_authentication(funding_keypair, auth_keypair, url)).await;
