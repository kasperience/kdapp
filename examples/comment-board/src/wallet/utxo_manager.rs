...
impl UtxoLockManager {
    ...
    pub async fn create_script_based_bond(
        ...
        if let Some((outpoint, entry)) = self.available_utxos.first().cloned() {
            if entry.amount >= bond_amount + 1000 {
                // Need extra for fees
    ...
    async fn create_script_bond_transaction(
        ...
        use crate::utils::{FEE, PATTERN, PREFIX};
    ...
    pub async fn split_large_utxo(&mut self, max_utxo_size: u64) -> Result<(), String> {
        ...
        if let Some((outpoint, entry)) = large_utxo.cloned() {
            ...
            info!(
                "🔄 MICRO-SPLITTING large UTXO: {:.6} KAS -> {} tiny chunks (mass limit solution)",
                entry.amount as f64 / 100_000_000.0,
                num_outputs
            );

            if chunk_size < 10_000 {
                // Less than 0.0001 KAS per chunk
                return Err("UTXO too small to split into micro-UTXOs".to_string());
            }

            info!(
                "📦 Creating {} UTXOs of ~{:.6} KAS each (minimizing transaction mass)",
                num_outputs,
                chunk_size as f64 / 100_000_000.0
            );
            ...
            for _ in 0..num_outputs {
                outputs.push(TransactionOutput { value: chunk_size, script_public_key: script.clone() });
            }

            info!(
                "🔬 Creating {} micro-UTXOs of {:.6} KAS each (total: {:.6} KAS)",
                outputs.len(),
                chunk_size as f64 / 100_000_000.0,
    ...
    pub fn unlock_bond(&mut self, comment_id: u64) -> Result<u64, String> {
        ...
        if let Some(locked_utxo) = self.locked_utxos.remove(&comment_id) {
            ...
            info!("🔓 Unlocked {:.6} KAS bond for comment {}", locked_utxo.bond_amount as f64 / 100_000_000.0, comment_id);
